...
parser.add_argument('--labelling', type=str, default="multiclass", metavar="LABELLING",
  help="how to label phishing [any/infected/generated/multiclass] (default: multiclass)")
parser.add_argument('--balancing', type=int, metavar="BALANCING", default=0,
  help="coefficient for balancing (default: 0)")
...
urls, labels, _ = read_data(FLAGS["data.data_dir"], FLAGS["labelling"]) 
...
for i in range(len(labels)):
    ...
    if label == 1: 
        ...
        neg_x.append(i)
print("Overall GEN/INF/ben split: {}/{}/{}".format(len(gen_x), len(inf_x), len(neg_x)))
gen_x = np.array(gen_x) 
inf_x = np.array(inf_x) 
neg_x = np.array(neg_x) 

x_train, y_train, x_test, y_test = prep_train_test(gen_x, inf_x, neg_x, FLAGS["data.dev_pct"])
