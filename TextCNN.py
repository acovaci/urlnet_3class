...
class TextCNN(object): 
    def __init__(self, char_ngram_vocab_size, word_ngram_vocab_size, char_vocab_size, \
        ...
        filter_sizes=[3,4,5,6], mode=0, balancing=0): 
        if mode == 4 or mode == 5: 
        ...
        with tf.name_scope("output"): 
            ...
            w = tf.get_variable("w", shape=(128, 3), initializer=tf.contrib.layers.xavier_initializer()) 
        ...
        with tf.name_scope("loss"): 
            ...
            losses *= 1 + (1 - tf.cast(tf.equal(self.predictions, 0), tf.float32)) * tf.cast(tf.equal(tf.argmax(self.input_y, 1), 0), tf.float32) * balancing
