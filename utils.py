...
def read_data(file_dir, phishing="any"): 
        ...
        for line in file.readlines(): 
            ...
            if phishing == "any":
                if label > 0: 
                    ...
                    labels.append(0) 
            if phishing == "generated":
                if label == 1: 
                    labels.append(1) 
                else: 
                    labels.append(0) 
            if phishing == "infected":
                if label == 2: 
                    labels.append(1) 
                else: 
                    labels.append(0) 
            if phishing == "multiclass":
                labels.append(label if label != -1 else 0) 
            url = items[1][:-1]
            urls.append(url) 
    return urls, labels, original_labels
...
def prep_train_test(gen_x, inf_x, neg_x, dev_pct): 
    np.random.seed(10) 
    shuffle_indices=np.random.permutation(np.arange(len(gen_x)))
    gen_x_shuffled = gen_x[shuffle_indices]
    dev_idx = -1 * int(dev_pct * float(len(gen_x)))
    gen_train = gen_x_shuffled[:dev_idx]
    gen_test = gen_x_shuffled[dev_idx:]

    np.random.seed(10) 
    shuffle_indices=np.random.permutation(np.arange(len(inf_x)))
    inf_x_shuffled = inf_x[shuffle_indices]
    dev_idx = -1 * int(dev_pct * float(len(inf_x)))
    inf_train = inf_x_shuffled[:dev_idx]
    inf_test = inf_x_shuffled[dev_idx:]
    ...
    x_train = np.array(list(gen_train) + list(inf_train) + list(neg_train))
    y_train = len(gen_train)*[1] + len(inf_train)*[2] + len(neg_train)*[0]
    x_test = np.array(list(gen_train) + list(inf_train) + list(neg_test))
    y_test = len(gen_train)*[1] + len(inf_train)*[2] + len(neg_train)*[0]
    ...
    print("Train GEN/INF/ben split: {}/{}/{}".format(len(gen_test), len(inf_test), len(neg_train)))
    print("Test GEN/INF/ben split: {}/{}/{}".format(len(gen_test), len(inf_test), len(neg_test)))
